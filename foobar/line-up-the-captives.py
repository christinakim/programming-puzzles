...
def positioning(n, r):
    #r is the bunnies seen from the right, n = num of bunnies
    if r > n:
    ...
    return memoize(
        (n, r),
        lambda: positioning(n - 1, r - 1) + positioning(n - 1, r) * (n - 1),
    )
        

def answer(x, y, n):

    return positioning(n - 1, x + y - 2) * f(x+y-2)/(f(x-1)*f(x+y-2 -x +1))